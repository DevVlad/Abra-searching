...
class DropdownFieldDumb extends React.Component {
	...
	preRenderMethod(dataForRender, props, willmount) {
		...
		if (!dataForRender.verified && props.enableDev) {
			...
			if (props.handleUnknown && !willmount && props.value !== this.state.controlledVal && !this.state.typing) {
				...
				props.handleUnknown(props.value);
			...
			this.InsertedValueFromPropsToState(props.entityToText(dataForRender.found), props.value);
		}
	}

	calculateDataForRedner(props, state) {
		console.log('DropdownFieldDumb: calculateDataForRedner');
		const [ found ] = this.props.data.filter(e => this.props.entityToValue(e) === this.props.value);
		console.log(this.props.data, this.props.value);
		return {
			data: this.props.data.map(e => ({
				...e,
				_text: this.props.entityToText(e)
			})),
			found: found,
			verified: found ? this.props.value : undefined
		};
	...
	handleOnSelect(e) {
		console.log('DropdownFieldDumb: handleOnSelect', e);
	...
	// 	return {
	// 		data: props.data,
	// 		value: props.value && verificationOfData ? props.value : null,
	// 		verified: verificationOfData
	// 	};
	// }
	...
	render() {
		console.log('DropdownFieldDumb: render', this.state);
		...
		return (
			<div id={`DropdownFieldDumb_${this.props.alias}`}>
				<AbstractAutoComplete
					...
					errorText={ this.props.errorText || this.props.warnText || this.props.passText }
					errorStyle={ { color: color } }
					dataSourceConfig={ { text: '_text', value: '_text' } }
					dataSource={ data }
